...
**Laundering activity is heavily time-concentrated, not evenly distributed**
![Laundering rate over time](./docs/results/laundering_rate_over_time.png)

Transaction volume is highest early in the time window and tapers off, while the normalized laundering rate stays near zero for most of the period then spikes sharply after mid-September. This concentration is exactly why a naive time-based train/test split produced a 10x imbalance in laundering rate between splits — and why we moved to a class-stratified split instead.

**Laundering accounts sit inside a dense transactional neighborhood**
![Sample account neighborhood](./docs/results/sample_neighborhood.png)

A sample laundering account (orange) shows a large fan of connected accounts and several self-loops, consistent with structuring behavior. This kind of local graph structure is what the GNN is expected to pick up on, and what PGExplainer will later be asked to highlight.

### Challenges & Lessons Learned so far
- **Naive time-based splitting leaked class imbalance**: an initial quantile-based time split produced a test set with a ~10x higher laundering rate than train. Fixed by stratifying the split per class before merging.
- **Account IDs collide across banks**: 12 real cross-bank account ID collisions were found, confirming the need for a composite `Bank_Account` node key instead of `Account` alone.
- **Scale**: the full dataset (31.9M transactions) makes even simple feature computations (e.g. per-account aggregates) memory-intensive; intermediate arrays like `edge_attr.npy` are multiple GB, so they are not checked into GitHub — they're regenerated by running the notebooks.
- **Leakage discipline required constant vigilance**: several candidate features (e.g. `format_risk_score`, whole-dataset `amount_zscore`) had to be excluded or reworked because they were fit across train+val+test instead of train-only.
